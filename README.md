...
Last updated: August 18, 2026 (code-review audit: `user-contract` + `user-service` modules complete; `order-service` 12/15 files, 4 real findings so far — 2 fixed, 2 confirmed and queued for the files that own them)
...
**At a glance:** 99/99 tests passing across `user-contract` + `user-service` + `order-service` + `payment-service` + `restaurant-service` + `api-gateway-service` — see the **[consolidated test report](https://terrence721.github.io/saga-full/test-report.html)**, a single file that CI keeps current on every push to `main` as more test cases are added. To generate it locally instead, run `./gradlew test --continue && ./gradlew aggregateTestReport` — see [CONTRIBUTING.md](CONTRIBUTING.md#consolidated-test-report-all-modules-one-file) for details.
...
- **[`docs/code-review.md`](docs/code-review.md)** — a per-module, per-file code-review audit of the whole codebase, one real GitHub PR per file (findings or not). `user-contract` and `user-service` are complete (5 real findings fixed, including two real security issues); `order-service` is 12/15 files in, with a real IDOR fix, a DTO-validation fix, and two more findings traced end-to-end and queued for the files that own them — see [todo.md](todo.md) for the tracking table.
