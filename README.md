...
Last updated: September 2, 2026 (code-review audit: `api-gateway-service`'s audit in progress, 11/15 files done — a real event-loop-blocking concurrency bug found and fixed in `AuthenticationController.java`, plus real test-coverage gaps fixed in `GatewayFallbackController.java` and `GlobalExceptionHandler.java`; 23 real findings fixed audit-wide so far)
...
- **[`docs/code-review.md`](docs/code-review.md)** — a per-module, per-file code-review audit of the whole codebase, one real GitHub PR per file (findings or not). `user-contract`, `user-service`, `order-service`, `payment-service`, and `restaurant-service` are complete; `api-gateway-service` is now in progress, the last of the 6 modules, 11/15 files done — a real event-loop-blocking concurrency bug found and fixed in `AuthenticationController.java`, plus real test-coverage gaps in `GatewayFallbackController.java` and `GlobalExceptionHandler.java`. 23 real findings fixed audit-wide so far, including three real security issues, a Kafka poison-pill fix, a payment-decline path that never existed, and an unlocked stock-deduction race that could oversell inventory once the service scales past one replica — see [todo.md](todo.md) for the tracking table.
