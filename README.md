...
Last updated: September 14, 2026 (register/POS frontend build complete — all 9 steps done, backend through tests through CI; a real Docker build-concurrency bug found and fixed; a separate DRY/SOLID/composition audit closed with 0 findings left open)
...
**At a glance:** 181/181 tests passing across `user-contract` + `user-service` + `order-service` + `payment-service` + `restaurant-service` + `api-gateway-service` — see the **[consolidated test report](https://terrence721.github.io/saga-full/test-report.html)**, a single file that CI keeps current on every push to `main` as more test cases are added. To generate it locally instead, run `./gradlew test --continue && ./gradlew aggregateTestReport` — see [CONTRIBUTING.md](CONTRIBUTING.md#consolidated-test-report-all-modules-one-file) for details. The `frontend/` module has its own separate suite (20 Vitest + React Testing Library tests, `cd frontend && yarn test`), run by CI's own `Frontend` job but not yet folded into the aggregated report above.
...
The rest of the [wiki](https://github.com/Terrence721/saga-full/wiki) goes deeper per-module (one page per service, plus the frontend).
...
- **[`docs/code-review.md`](docs/code-review.md)** — a per-module, per-file code-review audit of the whole codebase, one real GitHub PR per file (findings or not). **Complete: all 6 modules, 73/73 files, 26 real findings fixed across 26 files, 0 findings left open** — including three real security issues, a Kafka poison-pill fix caught three separate times as the same module pattern repeated, a payment-decline path that never existed, an unlocked stock-deduction race that could oversell inventory once a service scales past one replica, an event-loop-blocking gRPC call on the gateway's login path, and multiple genuine test-coverage gaps closed with real assertions — see [todo.md](todo.md) for the full tracking table.
...
```text
  ...
  frontend/              register/POS UI (React/TS/Vite/Tailwind)          ✅ done
```
