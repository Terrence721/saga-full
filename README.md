...
Last updated: September 4, 2026 (code-review audit: `api-gateway-service`'s audit in progress, 14/15 files done — a real event-loop-blocking concurrency bug found and fixed in `AuthenticationController.java`, real test-coverage gaps fixed in `GatewayFallbackController.java`, `GlobalExceptionHandler.java`, and `UserGrpcClient.java`, and real CWE-117 log-injection fixes in `JwtPerimeterGuardGatewayFilterFactory.java` and `UserGrpcClient.java`; 26 real findings fixed audit-wide so far)
...
**At a glance:** 135/135 tests passing across `user-contract` + `user-service` + `order-service` + `payment-service` + `restaurant-service` + `api-gateway-service` — see the **[consolidated test report](https://terrence721.github.io/saga-full/test-report.html)**, a single file that CI keeps current on every push to `main` as more test cases are added. To generate it locally instead, run `./gradlew test --continue && ./gradlew aggregateTestReport` — see [CONTRIBUTING.md](CONTRIBUTING.md#consolidated-test-report-all-modules-one-file) for details.
...
- **[`docs/code-review.md`](docs/code-review.md)** — a per-module, per-file code-review audit of the whole codebase, one real GitHub PR per file (findings or not). `user-contract`, `user-service`, `order-service`, `payment-service`, and `restaurant-service` are complete; `api-gateway-service` is now in progress, the last of the 6 modules, 14/15 files done — a real event-loop-blocking concurrency bug found and fixed in `AuthenticationController.java`, real test-coverage gaps in `GatewayFallbackController.java`, `GlobalExceptionHandler.java`, and `UserGrpcClient.java`, and real CWE-117 log-injection fixes in `JwtPerimeterGuardGatewayFilterFactory.java` and `UserGrpcClient.java`. 26 real findings fixed audit-wide so far, including three real security issues, a Kafka poison-pill fix, a payment-decline path that never existed, and an unlocked stock-deduction race that could oversell inventory once the service scales past one replica — see [todo.md](todo.md) for the tracking table.
